...
class Openvote_Vote {
    ...
    private static function table(): string {
        global $wpdb;
        return $wpdb->prefix . 'openvote_votes';
    }

    /**
     * Check if a user is eligible to vote in a poll.
     * Delegates to Openvote_Eligibility::can_vote_or_error() (7 checks).
     *
     * @return true|\WP_Error
     */
    public static function is_eligible( int $poll_id, int $user_id ): true|\WP_Error {
        return Openvote_Eligibility::can_vote_or_error( $user_id, $poll_id );
    }
    ...
    public static function get_eligible_count( object $poll ): int {
        ...
        // --- Group filter via wp_openvote_group_members ---
    ...
    public static function cast( int $poll_id, int $user_id, array $answers, bool $is_anonymous = false ): true|\WP_Error {
        ...
        if ( ! $poll ) {
            return new \WP_Error( 'poll_not_found', __( 'Głosowanie nie zostało znalezione.', 'openvote' ), [ 'status' => 404 ] );
        }

        if ( ! Openvote_Poll::is_active( $poll ) ) {
            return new \WP_Error( 'poll_not_active', __( 'Głosowanie nie jest aktywne.', 'openvote' ), [ 'status' => 403 ] );
        }
        ...
        foreach ( $question_ids as $qid ) {
            if ( ! isset( $answers[ $qid ] ) ) {
                return new \WP_Error( 'missing_answer', __( 'Odpowiedz na wszystkie pytania.', 'openvote' ), [ 'status' => 400 ] );
            }
        }
        ...
        foreach ( $answers as $question_id => $answer_id ) {
            ...
            if ( ! in_array( $question_id, $question_ids, true ) ) {
                return new \WP_Error( 'invalid_question', __( 'Nieprawidłowe pytanie.', 'openvote' ), [ 'status' => 400 ] );
            }
    ...
    public static function get_voters_admin( int $poll_id, int $limit = 0, int $offset = 0 ): array {
        ...
        $fn_core   = Openvote_Field_Map::is_core_field( $map['first_name'] ?? 'first_name' );
        $ln_core   = Openvote_Field_Map::is_core_field( $map['last_name'] ?? 'last_name' );
        ...
        foreach ( $rows as $row ) {
            ...
            if ( '' === $name ) {
                $name = __( '(brak imienia i nazwiska)', 'openvote' );
            }
