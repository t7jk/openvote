...
class Evoting_Survey {
    ...
    private static function surveys_table(): string {
        global $wpdb;
        return $wpdb->prefix . 'evoting_surveys';
    }

    private static function questions_table(): string {
        global $wpdb;
        return $wpdb->prefix . 'evoting_survey_questions';
    }

    private static function responses_table(): string {
        global $wpdb;
        return $wpdb->prefix . 'evoting_survey_responses';
    }

    private static function answers_table(): string {
        global $wpdb;
        return $wpdb->prefix . 'evoting_survey_answers';
    }
    ...
    public static function duplicate( int $survey_id ): int|false {
        ...
        $new_id = self::create( [
            'title'       => $original->title . ' ' . __( '(kopia)', 'evoting' ),
     ...
     *
     * @param int    $response_id ID rekordu w wp_evoting_survey_responses.
    ...
    public static function get_user_profile_data( int $user_id ): array {
        ...
        $phone = '';
        if ( class_exists( 'Evoting_Field_Map' ) ) {
            $map = Evoting_Field_Map::get();
            if ( ! empty( $map['phone'] ) && $map['phone'] !== Evoting_Field_Map::NOT_SET_KEY ) {
    ...
    public static function check_user_profile( int $user_id ): array {
        ...
        $required = Evoting_Field_Map::get_survey_required_fields();
        $missing  = [];
