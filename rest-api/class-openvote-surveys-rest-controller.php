...
class Openvote_Surveys_Rest_Controller {
    ...
    public function get_surveys( WP_REST_Request $request ): WP_REST_Response {
        $surveys = Openvote_Survey::get_all( [ 'status' => 'open' ] );
    ...
    public function get_survey( WP_REST_Request $request ): WP_REST_Response|WP_Error {
        ...
        if ( ! $survey ) {
            return new WP_Error( 'survey_not_found', __( 'Ankieta nie istnieje.', 'openvote' ), [ 'status' => 404 ] );
        }

        $user_id  = get_current_user_id();
        $response = $user_id ? Openvote_Survey::get_user_response( $survey_id, $user_id ) : null;
    ...
    public function submit_response( WP_REST_Request $request ): WP_REST_Response|WP_Error {
        ...
        if ( ! Openvote_Survey::is_active( $survey ) ) {
            return new WP_Error( 'survey_closed', __( 'Ankieta nie jest aktualnie aktywna.', 'openvote' ), [ 'status' => 403 ] );
        }
        ...
        if ( ! $profile_check['ok'] ) {
            return new WP_Error(
                'incomplete_profile',
                sprintf(
                    __( 'Uzupełnij profil przed wypełnieniem ankiety. Brakujące pola: %s', 'openvote' ),
        ...
        $user_data = Openvote_Survey::get_user_profile_data( $user_id );

        $saved = Openvote_Survey::save_response( $survey_id, $user_id, $clean_answers, $response_status, $user_data );

        if ( ! $saved ) {
            return new WP_Error( 'save_error', __( 'Błąd zapisu odpowiedzi.', 'openvote' ), [ 'status' => 500 ] );
        }
        ...
        return rest_ensure_response( [
            ...
            'message' => 'ready' === $response_status
                ? __( 'Twoja odpowiedź została zapisana jako Gotowa.', 'openvote' )
                : __( 'Twoja odpowiedź została zapisana jako Szkic.', 'openvote' ),
        ] );
    }
    ...
    public function get_my_response( WP_REST_Request $request ): WP_REST_Response|WP_Error {
        ...
        $response = Openvote_Survey::get_user_response( $survey_id, $user_id );
    ...
    private function format_survey( object $survey, bool $include_questions ): array {
        ...
        if ( $include_questions ) {
            $questions = Openvote_Survey::get_questions( (int) $survey->id );
    ...
    public function complete_profile( WP_REST_Request $request ): WP_REST_Response {
        ...
        if ( empty( $fields ) ) {
            return new WP_REST_Response( [ 'success' => false, 'message' => __( 'Brak danych do zapisania.', 'openvote' ) ], 400 );
        }

        $allowed = array_keys( Openvote_Field_Map::DEFAULTS );
        ...
        foreach ( $fields as $logical => $raw_value ) {
            ...
            $actual_key = Openvote_Field_Map::get_field( $logical );
            ...
            if ( 'email' === $logical ) {
                if ( ! is_email( $value ) ) {
                    return new WP_REST_Response(
                        [ 'success' => false, 'message' => __( 'Nieprawidłowy adres e-mail.', 'openvote' ) ],
                        400
                    );
                }
                ...
                if ( 'phone' === $logical ) {
                    update_user_meta( $user_id, 'openvote_phone', $value );
                    $map = Openvote_Field_Map::get();
                    if ( Openvote_Field_Map::NOT_SET_KEY === $map['phone'] ) {
                        $map['phone'] = 'openvote_phone';
                        update_option( Openvote_Field_Map::OPTION_KEY, $map, false );
                    }
                }
                // Pozostałe niezmapowane pola — pomijamy.

            } elseif ( Openvote_Field_Map::is_core_field( $actual_key ) ) {
