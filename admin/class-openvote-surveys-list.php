...
class Openvote_Surveys_List extends WP_List_Table {

    private string $current_status = '';

    public function __construct() {
        parent::__construct( [
            'singular' => __( 'ankieta', 'openvote' ),
            'plural'   => __( 'ankiety', 'openvote' ),
            'ajax'     => false,
        ] );
    ...
    protected function get_views(): array {
        ...
        $labels = [
            ...
            'open'   => __( 'Otwarta', 'openvote' ),
            'closed' => __( 'Zamknięta', 'openvote' ),
        ];

        $base_url = admin_url( 'admin.php?page=openvote-surveys' );
        $views    = [];
    ...
    protected function column_title( $item ): string {
        $edit_url  = admin_url( 'admin.php?page=openvote-surveys&action=edit&survey_id=' . $item->id );
        $view_url  = admin_url( 'admin.php?page=openvote-surveys&action=view&survey_id=' . $item->id );
        $resp_url  = admin_url( 'admin.php?page=openvote-surveys&action=responses&survey_id=' . $item->id );
        $del_url   = wp_nonce_url(
            admin_url( 'admin.php?page=openvote-surveys&action=delete&survey_id=' . $item->id ),
            'openvote_delete_survey_' . $item->id
        );
        $close_url = wp_nonce_url(
            admin_url( 'admin.php?page=openvote-surveys&action=close&survey_id=' . $item->id ),
            'openvote_close_survey_' . $item->id
        );
        $dup_url   = wp_nonce_url(
            admin_url( 'admin.php?page=openvote-surveys&action=duplicate&survey_id=' . $item->id ),
            'openvote_duplicate_survey_' . $item->id
        );
        ...
        if ( 'open' === $item->status ) {
            ...
            $actions['responses'] = sprintf(
                '<a href="%s">%s</a>',
                esc_url( $resp_url ),
                esc_html__( 'Zgłoszenia', 'openvote' )
            );
            $actions['close'] = sprintf(
                '<a href="%s" onclick="return confirm(\'%s\');">%s</a>',
                esc_url( $close_url ),
                esc_js( __( 'Zakończyć ankietę? Operacja nieodwracalna.', 'openvote' ) ),
        ...
        if ( 'closed' === $item->status ) {
            ...
            $actions['delete'] = sprintf(
                ...
                esc_url( $del_url ),
                esc_js( __( 'Usunąć tę ankietę wraz z odpowiedziami?', 'openvote' ) ),
        ...
        $counts = Openvote_Survey::get_response_counts_for_survey( (int) $item->id );
        $n      = $counts['not_spam'];
        $y      = $counts['total'];
        $counts_html = ( $n !== 0 || $y !== 0 )
            ? ' <span class="openvote-survey-row-counts">(' . (int) $n . '/' . (int) $y . ')</span>'
            : '';
    ...
    public function process_bulk_action(): void {
        ...
        if ( ! isset( $_POST['openvote_survey_bulk_nonce'] ) ||
             ! check_admin_referer( 'openvote_survey_bulk_action', 'openvote_survey_bulk_nonce' ) ) {
            return;
        }
        ...
        if ( 'close' === $action ) {
            ...
            foreach ( $ids as $id ) {
                $s = Openvote_Survey::get( $id );
                if ( $s && 'open' === $s->status ) {
                    Openvote_Survey::update( $id, [ 'status' => 'closed', 'date_end' => $now ] );
                }
            }
            wp_safe_redirect( add_query_arg( [ 'page' => 'openvote-surveys', 'bulk_closed' => count( $ids ) ], admin_url( 'admin.php' ) ) );
            exit;
        }

        if ( 'delete' === $action ) {
            foreach ( $ids as $id ) {
                Openvote_Survey::delete( $id );
            }
            wp_safe_redirect( add_query_arg( [ 'page' => 'openvote-surveys', 'bulk_deleted' => count( $ids ) ], admin_url( 'admin.php' ) ) );
            exit;
        }
    }
    ...
    public function display(): void {
        wp_nonce_field( 'openvote_survey_bulk_action', 'openvote_survey_bulk_nonce' );
        parent::display();
    }
    ...
    private static function get_surveys_with_search( array $args ): array {
        ...
        $table  = $wpdb->prefix . 'openvote_surveys';
        $where  = '1=1';
        $params = [];
