...
class Evoting_Admin_Surveys {

    public function handle_form_submission(): void {
        if ( ! isset( $_POST['evoting_survey_nonce'] ) ) {
            return;
        }

        if ( ! check_admin_referer( 'evoting_save_survey', 'evoting_survey_nonce' ) ) {
            return;
        }

        if ( ! Evoting_Admin::user_can_access_coordinators() ) {
        ...
        if ( is_wp_error( $data ) ) {
            set_transient( 'evoting_survey_admin_error', $data->get_error_message(), 30 );
            wp_safe_redirect( wp_get_referer() ?: admin_url( 'admin.php?page=evoting-surveys' ) );
            exit;
        }

        // Przycisk submit określa status.
        $submit_action = sanitize_text_field( $_POST['evoting_submit_action'] ?? 'draft' );
        $data['status'] = ( 'start_now' === $submit_action ) ? 'open' : 'draft';

        if ( $survey_id ) {
            Evoting_Survey::update( $survey_id, $data );
            $redirect = add_query_arg( 'updated', 1, admin_url( 'admin.php?page=evoting-surveys' ) );
        } else {
            $new_id = Evoting_Survey::create( $data );
            if ( false === $new_id ) {
                set_transient( 'evoting_survey_admin_error', __( 'Błąd zapisu ankiety.', 'evoting' ), 30 );
                wp_safe_redirect( admin_url( 'admin.php?page=evoting-surveys&action=new' ) );
                exit;
            }
            $redirect = admin_url( 'admin.php?page=evoting-surveys&created=1' );
        }
    ...
    private function sanitize_form_data(): array|\WP_Error {
        ...
        if ( ! isset( $duration_seconds[ $duration_key ] ) ) {
            return new \WP_Error( 'invalid_duration', __( 'Wybierz poprawny czas trwania ankiety.', 'evoting' ) );
        }
        ...
        foreach ( array_values( $raw_questions ) as $q ) {
            ...
            $allowed_profile = array_keys( Evoting_Field_Map::DEFAULTS );
        ...
        if ( count( $questions ) < 1 ) {
            return new \WP_Error( 'no_questions', __( 'Dodaj przynajmniej jedno pole ankiety.', 'evoting' ) );
        }
        if ( count( $questions ) > Evoting_Survey::MAX_QUESTIONS ) {
            return new \WP_Error(
                'too_many_questions',
                sprintf( __( 'Maksymalnie %d pól ankiety.', 'evoting' ), Evoting_Survey::MAX_QUESTIONS )
            );
        }
