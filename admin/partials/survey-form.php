...
$questions    = $is_edit ? Openvote_Survey::get_questions( (int) $survey->id ) : [];
...
$page_title = $is_read_only
    ...
    : ( $is_edit ? __( 'Edytuj ankietę', 'openvote' ) : __( 'Nowa ankieta', 'openvote' ) );
?>
<div class="wrap">
    ...
    <form method="post" action="" id="openvote-survey-form">
        ...
        <table class="form-table" style="max-width:700px;">
            <tr>
                <th scope="row"><label for="survey_title"><?php esc_html_e( 'Tytuł', 'openvote' ); ?> <span style="color:red">*</span></label></th>
                <td>
                    <input type="text"
                           id="survey_title"
                           name="survey_title"
            ...
            <tr>
                <th scope="row" style="vertical-align:top;padding-top:12px;">
                    <label for="survey_description"><?php esc_html_e( 'Opis', 'openvote' ); ?></label>
                </th>
                <td>
                    ...
                    <p class="description"><?php esc_html_e( 'Opisz cel ankiety (np. wybór kandydata na przewodniczącego ruchu).', 'openvote' ); ?></p>
                </td>
            </tr>
            <tr>
                <th scope="row"><label for="survey_date_start_display"><?php esc_html_e( 'Data i godzina rozpoczęcia', 'openvote' ); ?></label></th>
                <td>
                    ...
                    <p class="description"><?php esc_html_e( 'Zawsze „teraz". Przy „Wystartuj ankietę" data startu ustawiana jest na bieżący moment.', 'openvote' ); ?></p>
                </td>
            </tr>
            <tr>
                <th scope="row"><label for="survey_duration"><?php esc_html_e( 'Czas trwania ankiety', 'openvote' ); ?> <span style="color:red">*</span></label></th>
                <td>
                    ...
                    <p class="description"><?php esc_html_e( 'Od momentu startu ankiety.', 'openvote' ); ?></p>
                </td>
            </tr>
        </table>
        ...
        <h2 style="margin-top:28px;"><?php esc_html_e( 'Pola ankiety', 'openvote' ); ?>
            <span id="openvote-survey-field-count" style="font-size:.8em;font-weight:400;color:#666;margin-left:8px;">
                (<?php echo count( $questions ); ?> / <?php echo esc_html( Openvote_Survey::MAX_QUESTIONS ); ?>)
            </span>
        </h2>
        <p class="description" style="max-width:700px;">
            <?php esc_html_e( 'Stały nagłówek ankiety widoczny na stronie publicznej: "Proszę o uzupełnienie."', 'openvote' ); ?>
            <?php printf(
                esc_html__( 'Możesz dodać od 1 do %d pól.', 'openvote' ),
                esc_html( Openvote_Survey::MAX_QUESTIONS )
            ); ?>
        </p>

        <div id="openvote-survey-fields" style="max-width:700px;margin-top:12px;">
        ...
        <p style="margin-top:12px;">
            <button type="button" id="openvote-add-survey-field" class="button"
                    <?php echo count( $questions ) >= Openvote_Survey::MAX_QUESTIONS ? 'disabled' : ''; ?>>
                + <?php esc_html_e( 'Dodaj pole', 'openvote' ); ?>
            </button>
        </p>
...
function openvote_render_survey_field_row( int $i, ?object $q, array $labels, array $profile_options, bool $read_only ): void {
    ...
    <div style="display:flex;align-items:flex-start;gap:8px;margin-bottom:10px;padding:12px;background:#f9f9f9;border:1px solid #ddd;border-radius:4px;">
        ...
        <button type="button"
                ...
                title="<?php esc_attr_e( 'Usuń pole', 'openvote' ); ?>">✕</button>
        <?php endif; ?>
    </div>
    <?php
}
