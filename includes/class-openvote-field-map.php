 ...
 *
 * Stored as WP option: openvote_field_map
 */
class Openvote_Field_Map {

    const OPTION_KEY   = 'openvote_field_map';
    const NO_CITY_KEY  = '__openvote_no_city__';
    const WSZYSCY_NAME = 'Wszyscy';

    /** Option key for storing which logical fields are required for voting. */
    const REQUIRED_FIELDS_OPTION = 'openvote_required_fields';

    /** Option key for storing which logical fields are required for surveys. */
    const SURVEY_REQUIRED_FIELDS_OPTION = 'openvote_survey_required_fields';

    /** Sentinel value meaning "field not mapped — treat as empty". */
    const NOT_SET_KEY = '__openvote_not_set__';
