 ...
 * Treść strony jest zapisana w opcji openvote_law_content i edytowana
 * z poziomu panelu admina (Open Vote → Przepisy).
 */
defined( 'ABSPATH' ) || exit;

class Openvote_Law_Page {

    /** Domyślny slug publicznej strony przepisów. */
    const DEFAULT_SLUG = 'przepisy';

    /** Klucz WP Rewrite Query Var. */
    const QUERY_VAR = 'openvote_law';
    ...
    public static function get_slug(): string {
        $s = get_option( 'openvote_law_slug', self::DEFAULT_SLUG );
    ...
    /**
     * Rejestruje query var "openvote_law" w WP.
     *
     * @param string[] $vars
     * @return string[]
     */
    ...
    public static function filter_template( string $template ): string {
        ...
        return OPENVOTE_PLUGIN_DIR . 'public/views/law-page.php';
    }
    ...
    public static function filter_document_title( string $title ): string {
        return __( 'Przepisy prawne', 'openvote' ) . ' — ' . get_bloginfo( 'name' );
    }
