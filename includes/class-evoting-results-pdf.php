 ...
 */
class Evoting_Results_Pdf {

	/**
	 * Czy biblioteka PDF jest dostępna (vendor zainstalowany).
	 */
	public static function is_available(): bool {
		$autoload = EVOTING_PLUGIN_DIR . 'vendor/autoload.php';
	 ...
	 * @param array  $results         Wynik Evoting_Vote::get_results.
	 * @param array  $voters          Wynik Evoting_Vote::get_voters_admin.
	 * @param array  $non_voters_list Wynik Evoting_Vote::get_non_voters.
	 */
	public static function output_download( object $poll, array $results, array $voters, array $non_voters_list = [] ): void {
		if ( ! self::is_available() ) {
			wp_die( esc_html__( 'Generowanie PDF wymaga zainstalowanych zależności Composer (composer install).', 'evoting' ) );
		}
		...
		$pdf->Cell( 0, 5, __( 'Raport wyników głosowania', 'evoting' ) . '   |   ' . gmdate( 'd.m.Y H:i' ), 0, 1 );
		$pdf->SetX( 15 );
		$pdf->Ln( 3 );
		...
		$pdf->MultiCell( 0, 9, __( 'Wyniki:', 'evoting' ) . ' ' . $poll->title, 0, 'L', false, 1 );
		$pdf->SetFont( 'dejavusans', '', 9 );
		$pdf->SetTextColor( 100, 100, 100 );
		$pdf->Cell( 0, 6, __( 'Status:', 'evoting' ) . ' ' . $poll->status
			. '   |   ' . __( 'Okres:', 'evoting' ) . ' ' . $poll->date_start . ' — ' . $poll->date_end, 0, 1 );
		$pdf->SetTextColor( 0, 0, 0 );
		$pdf->Ln( 5 );

		// ── Frekwencja ─────────────────────────────────────────────────────
		self::section_header( $pdf, __( 'Frekwencja', 'evoting' ) );

		$w1 = 100; $w2 = 22; $w3 = 22;
		self::table_header_row( $pdf, [
			[ __( 'Kategoria', 'evoting' ),  $w1, 'L' ],
			[ __( 'Liczba',    'evoting' ),  $w2, 'C' ],
			[ __( 'Procent',   'evoting' ),  $w3, 'C' ],
		] );
		self::table_row( $pdf, [
			[ __( 'Uprawnionych do głosowania', 'evoting' ), $w1, 'L' ],
			[ (string) $total_eligible, $w2, 'C' ],
			[ '100%',                   $w3, 'C' ],
		] );
		self::table_row( $pdf, [
			[ __( 'Uczestniczyło w głosowaniu', 'evoting' ), $w1, 'L' ],
			[ (string) $total_voters,   $w2, 'C' ],
			[ $pct_voted . '%',         $w3, 'C' ],
		] );
		self::table_row( $pdf, [
			[ __( 'Nie uczestniczyło', 'evoting' ), $w1, 'L' ],
		...
		if ( ! empty( $results['questions'] ) ) {
			self::section_header( $pdf, __( 'Wyniki pytań', 'evoting' ) );
			...
			foreach ( $results['questions'] as $i => $q ) {
				...
				self::table_header_row( $pdf, [
					[ __( 'Odpowiedź', 'evoting' ), $wa, 'L' ],
					[ __( 'Głosy',     'evoting' ), $wb, 'C' ],
					[ __( 'Procent',   'evoting' ), $wc, 'C' ],
				] );
		...
		self::section_header( $pdf, sprintf( __( 'Lista głosujących (%d)', 'evoting' ), $total_v_count ) );

		if ( ! empty( $voters ) ) {
			$pdf->SetFont( 'dejavusans', '', 8 );
			$pdf->SetTextColor( 100, 100, 100 );
			$pdf->Cell( 0, 5, __( 'Imię i nazwisko oraz zanonimizowany e-mail. Pozostałe dane utajnione.', 'evoting' ), 0, 1 );
			$pdf->SetTextColor( 0, 0, 0 );
			$pdf->Ln( 1 );

			$wv1 = 65; $wv2 = 75; $wv3 = 35;
			self::table_header_row( $pdf, [
				[ __( 'Imię i Nazwisko',         'evoting' ), $wv1, 'L' ],
				[ __( 'E-mail (zanonimizowany)',  'evoting' ), $wv2, 'L' ],
				[ __( 'Data głosowania',          'evoting' ), $wv3, 'C' ],
			] );
			foreach ( $voters as $v ) {
				self::table_row( $pdf, [
					[ $v['name']       ?? '',  $wv1, 'L' ],
			...
			$pdf->Cell( 0, 6, __( 'Nikt nie głosował.', 'evoting' ), 0, 1 );
		}
		$pdf->Ln( 7 );

		// ── Lista niegłosujących ───────────────────────────────────────────
		$total_nv_count = count( $non_voters_list );
		self::section_header( $pdf, sprintf( __( 'Nie głosowali (%d)', 'evoting' ), $total_nv_count ) );

		if ( ! empty( $non_voters_list ) ) {
			$pdf->SetFont( 'dejavusans', '', 8 );
			$pdf->SetTextColor( 100, 100, 100 );
			$pdf->Cell( 0, 5, __( 'Uprawnieni użytkownicy, którzy nie oddali głosu. Pseudonimy zanonimizowane.', 'evoting' ), 0, 1 );
			$pdf->SetTextColor( 0, 0, 0 );
			$pdf->Ln( 1 );
			...
			$pdf->SetFont( 'dejavusans', 'I', 9 );
			$pdf->Cell( 0, 6, __( 'Wszyscy uprawnieni użytkownicy oddali głos.', 'evoting' ), 0, 1 );
		}
		...
		$pdf->Cell( 0, 5, $brand_short . ' — ' . $brand_full . '   |   ' . __( 'Wygenerowano:', 'evoting' ) . ' ' . gmdate( 'd.m.Y H:i:s' ), 0, 1, 'C' );
